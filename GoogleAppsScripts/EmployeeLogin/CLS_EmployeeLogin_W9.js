...
function submitW9Form(workerId, w9Data, device) {
  try {
    ...
    if (workerData.w9Status === "approved") {
      return {
        ok: false,
        message: "You already have an approved W-9 on file.",
      };
    }
    ...
    if (!pdfResult.ok) {
      ...
      return {
        ok: false,
        message: "Failed to generate W-9 PDF. Please try again.",
      };
    }
    ...
    return {
      ok: false,
      message: "An error occurred while submitting your W-9. Please try again.",
    };
  }
}
  ...
  for (let i = 1; i < data.length; i++) {
    if (data[i][workerIdIdx] === workerId) {
      ...
      if (updates.w9Status !== undefined) {
        workersSheet
          .getRange(rowNum, w9StatusIdx + 1)
          .setValue(updates.w9Status);
      }
      if (updates.w9SubmittedDate !== undefined) {
        workersSheet
          .getRange(rowNum, w9SubmittedDateIdx + 1)
          .setValue(updates.w9SubmittedDate);
      }
      if (updates.w9ApprovedDate !== undefined) {
        workersSheet
          .getRange(rowNum, w9ApprovedDateIdx + 1)
          .setValue(updates.w9ApprovedDate);
      }
      if (updates.w9ApprovedBy !== undefined) {
        workersSheet
          .getRange(rowNum, w9ApprovedByIdx + 1)
          .setValue(updates.w9ApprovedBy);
      }
      if (updates.w9SsnLast4 !== undefined) {
        workersSheet
          .getRange(rowNum, w9SsnLast4Idx + 1)
          .setValue(updates.w9SsnLast4);
      }
      if (updates.w9PdfUrl !== undefined) {
        workersSheet
          .getRange(rowNum, w9PdfUrlIdx + 1)
          .setValue(updates.w9PdfUrl);
      }
  ...
  for (let i = 1; i < data.length; i++) {
    if (data[i][w9RecordIdIdx] === w9RecordId) {
      ...
      if (updates.approvedBy !== undefined) {
        w9RecordsSheet
          .getRange(rowNum, approvedByIdx + 1)
          .setValue(updates.approvedBy);
      }
      if (updates.approvedDate !== undefined) {
        w9RecordsSheet
          .getRange(rowNum, approvedDateIdx + 1)
          .setValue(updates.approvedDate);
      }
      if (updates.rejectionReason !== undefined) {
        w9RecordsSheet
          .getRange(rowNum, rejectionReasonIdx + 1)
          .setValue(updates.rejectionReason);
      }
 ...
 */
function sendW9SubmissionNotification_(
  workerId,
  displayName,
  w9RecordId,
  pdfUrl
  ...
  try {
    const adminEmail =
      PROPS.getProperty("CC_EMAIL") || "info@carolinalumpers.com";
        ...
        <div style="background-color: #f9f9f9; padding: 30px; border-radius: 0 0 8px 8px;">
          ...
          <div style="border: 2px solid #FFC107; border-radius: 8px; padding: 20px; margin: 20px 0; background-color: #FFFBF0;">
            ...
            <table style="width: 100%; border-collapse: collapse; font-size: 14px;">
              ...
              <tr><td style="padding: 5px 0;"><strong>Submitted:</strong></td><td>${new Date().toLocaleString(
                "en-US",
      ...
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="background-color: #f9f9f9; padding: 30px; border-radius: 0 0 8px 8px;">
          ...
          <div style="border: 2px solid #4CAF50; border-radius: 8px; padding: 20px; margin: 20px 0; background-color: #F1F8F4;">
            ...
            <table style="width: 100%; border-collapse: collapse; font-size: 14px;">
              <tr><td style="padding: 5px 0;"><strong>Record ID:</strong></td><td>${w9RecordId}</td></tr>
              <tr><td style="padding: 5px 0;"><strong>Approved:</strong></td><td>${new Date().toLocaleString(
                "en-US",
                { timeZone: "America/New_York" }
              )}</td></tr>
...
function generateW9PDF(workerId, w9Data) {
  try {
    ...
    if (!templateId || !folderId) {
      Logger.log(
        "⚠️ W9_TEMPLATE_ID or W9_FOLDER_ID not configured in Script Properties"
      );
    ...
    body.replaceText(
      "{{BackupWithholding}}",
      w9Data.backupWithholding ? "Yes" : "No"
    );
...
W-9 Email Testing System
  ...
  try {
    ...
    Logger.log(
      `💡 Make sure Gmail API is enabled in Project Settings > Services`
    );
...
function sendEmailWithGmailAPI_(recipient, subject, plainBody, htmlBody) {
  try {
    ...
    Logger.log(
      `⚠️ Falling back to GmailApp (emojis will be stripped from subject)...`
    );

    // Fallback to GmailApp (without emojis in subject)
    const subjectNoEmoji = subject
    ...
    Logger.log(
      `💡 To enable emojis in subjects, enable Gmail API in Project Settings > Services > Add Gmail API v1`
    );
  }
}
